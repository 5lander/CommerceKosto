...
| D-16.195 | (usuario) **El runbook de respaldos gana la restauración POR TENANT** (dump → base auxiliar → filas de una company → reinserción con RLS), documentada y probada sobre ensayo-b **antes de la parada del piloto**. La restauración completa queda para la pérdida total. *(pendiente: commit propio)* |
| D-16.196 | (usuario, sobre D-16.17/D-16.24) **El límite por destinatario es el principal; el de IP se calibra para IPs compartidas (CGNAT)**: umbral alto y solo 429, nunca bloqueo escalonado por IP sola. **Revisado: hoy sí puede** —25 fallos de una sola cuenta desde una IP bloquean esa IP, y la escalada llega a 60 minutos—, así que el eje de IP se relaja y se registra como incidencia. *(pendiente: commit propio)* |
