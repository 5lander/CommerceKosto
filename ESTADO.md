...
**Fase en curso:** **Pasada P16 → P20**. Commit 0 (Tooling) cerrado; **P16-A1 cerrado** (IVA en dos niveles + correo transaccional + límite de tasa + IP tras el proxy; `docs/pasos/P16-A1/`), con `npm run audit` en verde y `npm run bench` ejecutado. **P16-A2 cerrado** (sesión + token CSRF + frontera HTTP + lecturas de catálogo; `docs/pasos/P16-A2/`), con `npm run audit` en verde. **P16-B cerrado** (pricing + recipes/products + costing + concurrencia optimista; `docs/pasos/P16-B/`), con `npm run audit` en verde, `migrate:verify` 4/4 y `npm run bench` ejecutado **sobre su propio código** (hasta P16-B el bench medía el `dist` que hubiera en disco: INC-007, caso 12). **P16-C cerrado** (inventory + usuarios/roles/sucursales + `period.version`; `docs/pasos/P16-C/`): **los cuatro paquetes de API de la pasada están hechos**. **Armazón (pantalla 1) cerrado** (grupo de rutas `(app)`, permisos cerrados por defecto, mes en la URL, kit de lecturas y escrituras, las cuatro pantallas migradas; ADR-020 y ADR-022; `docs/pasos/P16/`), verificado entrando de verdad con tres roles a 1280 y 360 px: **destapó que «Entrar» no funcionaba desde un navegador sin sesión desde P16-A2 (INC-023)** y que una fila ya guardada de ventas o del conteo no se podía editar; los dos, arreglados. **P16-D cerrado** (el costeo marca `sinReceta` y el semáforo `SIN_DATO`; duda #12 cerrada por el usuario con la opción (a); `docs/pasos/P16-D/`). **Inicio (pantalla 2) cerrado** (resumen del mes o reposición según el permiso; `/` → `/inicio`; arreglo del signo en `lib/decimales` —INC-024— con las primeras pruebas de `apps/web`, ADR-027). **Pantalla 1b cerrada** (`/olvide` y `/restablecer`; INC-025, el 202 sin cuerpo; pruebas del transporte). **Pantalla 3 cerrada** (ventas lee la carta de `GET /productos/ubicaciones`; la rejilla cabe a 360 px). **Pantalla 4 cerrada** (`/insumos`: catálogo con costo de uso, sin costo para `BODEGA`). **Pantalla 5 cerrada** (`/insumos/nuevo`). **Pantalla 6 cerrada** (ficha, edición con versión y archivo). **Pantalla 7 cerrada** (`/grupos`, alta y edición). **Verificación multi-tenant cerrada** (D-16.193): el entorno sintético tiene **dos companies** y las pantallas 1–14 están comprobadas contra la de al lado. **D-16.199 cerrada** (el umbral del eje de IP sube a 50 cuentas/hora). **D-16.200 cerrada** (P16-H: anulación de una importación, y con ella **INC-029**, el dinero de una compra corregida que se contaba dos veces). **D-16.198 cerrada** (P16-G2: el simulacro pasa con libro y agosto cerrado; el runbook ya dice «probado», y la restauración por tenant sirve por fin para un cliente con meses cerrados). **D-16.201 cerrada** (P16-I: los tres agregados de dinero tienen caso conocido y prueba unitaria; la clase de INC-029 queda cerrada, y de paso sale la **duda abierta #16**). **D-16.202 registrada** (la duda #16 queda decidida y se construye como **P16-J, justo antes de la pantalla 25**). La base de desarrollo se reseteó y el entorno sintético está reconstruido: **los ids se resuelven por nombre** (`ids.mjs`), no escritos a mano. **D-16.202 confirmada y R15 registrada** (la identidad que ata la varianza de §16 con el inventario de §18; se construye en **P16-J**, justo antes de la pantalla 25). **Pantalla 15 cerrada** (`/productos/[id]/receta/versiones`, con `recipe.read` y las dos comprobaciones de tenant cruzado; con ella viaja la adenda de R15). **El proyecto está en MODO CIERRE desde el 2026-09-21** —las cinco reglas y el orden acordado están en su sección de arriba—, con las dudas **#9, #10, #11 y #13 decididas por el usuario**. **Pantalla 16 cerrada** (`/productos/[id]/propagar`, con `recipe.propagate` y la ida y vuelta de R11 probada contra la API). **Pantalla 17 cerrada** (`/movimientos`, con `inventory.read` que BODEGA no tiene, y el primer apunte de «Después del piloto»: el importe sin signo del libro). **Pantalla 18 cerrada** (`/movimientos/nuevo`, la pantalla de BODEGA; con ella, **INC-031** y la regla 48 de `audit:forbidden`). **Pantalla 19 cerrada** (`/movimientos/[id]/corregir`, con la corrección probada de punta a punta contra el libro y el saldo). **Pantallas 20 y 21 cerradas** (`/transferencias/nueva` y `/producciones/nueva`). **LA PASADA SE DETIENE AQUÍ POR LA REGLA 1 DEL MODO CIERRE: `INC-032` corrompe un número del libro** (un insumo sin precio a la fecha del lote vale `0,00` y la varianza de R10 dice que producir salió gratis) — **sigue esperando decisión**. El orden acordado con el usuario el 2026-09-22 es: **P16-E → P16-T (D-16.215, ADR-029) → pantalla 22**. **ADR-029 está escrito y propuesto; no se escribe código de P16-T hasta que el usuario lo lea.** Duda abierta **#14** (renombrar y archivar productos). Queda pendiente, y dicho, la prueba de `DELETE /usuarios/roles` a través de Caddy en modo producción (exige parar la pila local en uso). Dos cosas esperan al usuario y no bloquean el código: ratificar **D-16.51** y decidir sobre el **presupuesto del consolidado** (abajo, en dudas abiertas). El plan P0–P15 está completo y el sistema se ha desplegado entero en local. **P16-T cerrado (solo ADR-029, sin código).** **P16-U cerrado** (`docs/pasos/P16-U/`): al abrir el PR de la entrega se descubrió que **CI nunca había estado en verde** —dos corridas en toda la historia del repositorio, las dos rojas, con 51 commits acumulados entre ellas— porque el cluster arrancaba **sin los cuatro roles** (`INC-033`); arreglado por los dos lados, verificado con un volumen vacío de verdad, y convertido en la regla 49 de `audit:forbidden`. **Los 51 commits están publicados: `origin/main` deja de ir dieciocho días por detrás.**

**Lo que cambia para siempre a partir de aquí:** se **empuja por paquete, no por pasada**. Un guardián que nadie mira no es un guardián, y esa es la lección de INC-033 por encima de su causa técnica.

**Último commit:** `P16-U: la base arranca en limpio, y CI se pone en verde por primera vez`
...
| P16-H | **Anulación de una importación** (D-16.200): `inventory_movement.import_job_id`, `POST /importaciones/:id/anulacion` con `import.write` → filas de signo contrario en UNA transacción (R3), estado `ANULADA` con autor y fecha, 409 en mes cerrado · primer controlador de `imports` · 1 migración · **INC-029**: el importe de una compra corregida se duplicaba en `compras_del_mes` | ✅ 2026-09-20 · `npm run audit` **exit 0** · 🔴 importar→anular→saldo y dinero a cero, anular dos veces 409, mes cerrado 409, otra company 404, BODEGA 403, tipo no anulable 409 | *(el de este commit)* | — | — | D-16.200 |
| P16-G2 | **El simulacro con libro y mes cerrado** (D-16.198): períodos reabiertos y recerrados en la misma transacción con la lista en un parámetro de sesión (`costeo_app` no tiene `TEMP`), **líneas de conteo antes que su conteo** con la FK diferible solo ahí, verificación de que los meses cerrados vuelven · 2 migraciones · check **M12** (`search_path` de toda función) · **INC-030** · INC-007 caso 14 · señal de los 20 minutos en el runbook | ✅ 2026-09-20 · `npm run audit` **exit 0** · `migrate:verify` 4/4 · simulacro: **24 filas en 22 tablas**, agosto otra vez CERRADO, saldo 15 kg idéntico a la copia, `ensayo` intacto | *(el de este commit)* | — | — | D-16.198 |
| P16-I | **Tres casos conocidos para los tres agregados de dinero** (D-16.201): `inventory/domain/agregados.ts` pliega el libro de un período con la base apagada; **CC-010, CC-011 y CC-012** sobre un mismo mes con compra, corrección, transferencia, producción, merma, ajuste y consumo por venta; prueba de integración que ata el `SUM` de producción al pliegue · CLAUDE.md §3 gana la regla del código generado desde Python (INC-007) · **duda abierta #16** | ✅ 2026-09-20 · `npm run audit` **exit 0** · 7 unitarias nuevas en `agregados.spec.ts`, 4 en `vistas.spec.ts`, 1 de integración | *(el de este commit)* | — | — | D-16.201 · D-16.202 (ver la nota de abajo) |
| P16-I2 | **La regla del doble intérprete** en CLAUDE.md §3 (contenido con la herramienta de escritura o `-F`, nunca incrustado en el shell) · **base de desarrollo de 12 GB a 15 MB** y `costeo_restaurado` eliminada · entorno sintético reconstruido (dos companies) · **`ids.mjs`**: los guiones resuelven ids **por nombre** y los 21 con UUID caducos quedan marcados · **D-16.202 registrada** (duda #16 decidida) | ✅ 2026-09-20 · `npm run audit` **exit 0** · `seed:tenant` ×2, importación de los CSV sintéticos y `poblar-ensayo-b` verificados contra la API | *(el de este commit)* | — | — | D-16.202 |
| P16-I3 | **R15** en CLAUDE.md §6 y **E24** en la checklist: `varianza = −mermas_y_ajustes − diferencia`, una **identidad** y no una tolerancia · **CC-011 reescrito** con `4,00 = 2,50 merma − 0,50 ajustes + 2,00 sin explicar` y la identidad demostrada · tabla de cobertura con CC-010…012 y R15 · la prueba de hoy queda anclada como «hasta P16-J» | ✅ 2026-09-20 · `npm run audit` **exit 0** · sin código de aplicación: la regla se construye en **P16-J** | `42d9f1d` · **enmendado el 2026-09-21 sin commit propio**: R15 pasa a enunciarse en cantidades y gana la varianza de precio (ver la adenda de D-16.202) | — | — | D-16.202 |
| Pantalla 15 | `/productos/[id]/receta/versiones`: el historial de una receta en una sucursal, la más nueva arriba, con «Manda hoy» / «La más nueva» / «Deja el producto sin receta», nota y líneas con su unidad y su base · **pide `recipe.read`, no `recipe.write`** · enlace «Versiones» en la ficha y en la pantalla 14 · `componentes/recetas/versiones.ts` + `HistorialDeReceta.tsx` · **adenda de R15 y D-16.202** (CLAUDE.md §6, E24/E25, CC-013, sin commit propio) · **arreglo: la fila de P16-I rompía el tablero** | ✅ 2026-09-21 · types · lint · forbidden · complexity · duplication **en verde** (modo cierre, regla 2) · 3 roles × 1280/360 sin desborde · **tenant cruzado ✓** (producto de `ensayo-b` → «Ese producto no existe en tu company») · contrastado contra `GET /recetas/versiones` versión a versión | *(el de este commit)* | 8 (3 roles × 1280/360 + 2 de tenant cruzado) | — | D-16.203 |
| Pantalla 16 | `/productos/[id]/propagar`: previsualización con «Tiene receta propia» por sucursal (R11), propagación a las elegidas e historial con reversión · **pide `recipe.propagate`**, que solo tiene DUEÑA · `componentes/recetas/propagacion.ts`, `FormularioDePropagacion.tsx`, `HistorialDePropagaciones.tsx` · **`contexto.ts`** extraído a la tercera repetición (lo cazó `audit:duplication`) | ✅ 2026-09-21 · types · lint · forbidden · complexity · duplication **en verde** · 3 roles × 1280/360 sin desborde (gerente y bodega, sin acceso en sitio) · **tenant cruzado ✓** · **ida y vuelta real**: propia `[0.35]` → propagada `[0.2, 0.1, 0.02]` → revertida `[0.35]` **como versión nueva**, 3 versiones y nada borrado | *(el de este commit)* | 12 (3 roles × 1280/360 + cruzado + las 4 del ciclo propagar/revertir) | — | D-16.204, D-16.205 |
| Pantalla 17 | `/movimientos`: el libro de la sucursal elegida, lo más reciente primero, con filtros de insumo, tipo y rango de fechas **en la consulta a la API** y «Ver más» por cursor · «Corregido» / «Es una corrección» / «Sin desglose de IVA» (D-16.18) como estado · entrada nueva en la barra, con `inventory.read` que **BODEGA no tiene** (§4.3) · `componentes/inventario/{libro.ts, FiltrosDelLibro, TablaDelLibro}` | ✅ 2026-09-21 · types · lint · forbidden · complexity · duplication **en verde** · 3 roles × 1280/360 sin desborde · **el filtro se contrasta con la API**: `tipo=COMPRA` da 5 en las dos · el vacío **conserva los filtros** (4 controles) · **libro sintético sembrado** (el reseteo de P16-I2 no lo repobló) | *(el de este commit)* | 10 (3 roles × 1280/360 + filtro y vacío) | — | D-16.206, D-16.207 |
| Pantalla 18 | `/movimientos/nuevo`: compra, merma o ajuste, preguntando lo que cada tipo significa · «Total de la factura (con IVA)» (D-16.9) con presentación e IVA de la factura · **el permiso pasa del `layout.tsx` de sección a cada página**, porque BODEGA tiene `inventory.write` y no `inventory.read` · segunda entrada «Registrar» en la barra, **antes** del libro · `CampoDeCantidadConSigno` · **`INC-031`** y la regla **`lectura-con-funcion-estable`** de `audit:forbidden` (47 → 48) | ✅ 2026-09-21 · types · lint · forbidden · complexity · duplication **en verde** · 3 roles × 1280/360 sin desborde · **compra registrada DE VERDAD desde la pantalla, con BODEGA a 360 px**: fila nueva con `bruto 34,50 → neto 34,50`, tarifa `0` del artículo, `CONOCIDO`, fecha al `12:00Z` del día escrito · MERMA esconde los campos de compra y pide «Cuánto se perdió» | *(el de este commit)* | 9 (3 roles × 1280/360 + merma + compra antes y después) | — | D-16.208…D-16.210 |
| Pantalla 19 | `/movimientos/[id]/corregir`: la ficha de lo que se corrige y **un solo campo**, el motivo · los dos casos que no se pueden —ya corregido, o es una corrección— **dichos en sitio** · enlace «Corregir» en el libro solo donde tiene sentido · `inventory.read` para entrar, `inventory.write` para el formulario | ✅ 2026-09-22 · types · lint · forbidden · complexity · duplication **en verde** · 3 roles × 1280/360 sin desborde · **corrección hecha DE VERDAD desde la pantalla**: fila nueva `−25 kg` con la **misma fecha**, la original marcada `Corregido`, **10 filas donde había 9** y el saldo de Arroz de `125` a `100` · el gerente, fuera de su alcance, recibe «Esa ubicación no está en tu alcance» | *(el de este commit)* | 10 (3 roles × 1280/360 + los dos impedimentos + antes y después) | — | D-16.211, D-16.212 |
| Pantallas 20+21 | `/transferencias/nueva` (destino, insumo y cuánto; el par lo escribe la API en una transacción, R2) y `/producciones/nueva` (la preparación, el lote y **los insumos que de verdad entraron**) · dos entradas nuevas en la barra · `produccion.ts` con `preparacionesDe` y `lineasDeLaReceta` | ✅ 2026-09-22 · types · lint · forbidden · complexity · duplication **en verde** · 3 roles × 1280/360 sin desborde · **producción hecha DE VERDAD**: 3 insumos precargados de la receta **sin cantidad**, lote de `2 kg` al estándar (`16,00` = 2 × 8,00, R10), camarón de `32,5` a `31,2`, salsa de `0` a `2` · **dos hallazgos, abajo** | *(el de este commit)* | 15 (3 roles × 1280/360 × 2 pantallas + el lote y la transferencia) | — | D-16.213, D-16.214 |
| P16-T | **La transferencia en dos pasos sobre rutas de abastecimiento** (D-16.215): rutas declaradas, despacho con alcance sobre el origen, recepcion con alcance sobre el destino, **merma de traslado** con tipo propio, transito como proyeccion del libro y linea propia del consolidado, cierre de periodo sin bloqueo, aviso a los N dias, §4.3 sobre la respuesta cruda y los datos de la guia de remision sin emitirla · la pantalla 20 se rehace como dos | 🟡 **ADR-029 escrito y propuesto el 2026-09-22; el codigo espera a que el usuario lo lea** | | | | D-16.215 |
| Pantallas 22–27 | una fila por pantalla al commitear | ⬜ | | | | |
...
> **D-16.202, en largo** — decidida por el usuario el 2026-09-20 al cerrar P16-I, ampliada el 2026-09-21,
> y **se construye en P16-J, inmediatamente antes de la pantalla 25**. Vivía dentro de una celda del
> tablero y la rompía; está aquí porque es lo que hay que leer antes de escribir P16-J.
>
> El usuario **decide la duda #16 con la opción (a) extendida.** **`consumo_real` por ítem y ubicación deja de contar como consumo lo que salió por otra puerta**: `inicial + compras + transferencias recibidas − transferencias enviadas + entradas desde producción (al estándar) − salidas a producción − final físico`. La `varianza = consumo_real − consumo_teórico` se **desglosa** en «merma registrada» (atribuible) y «no atribuible» (el resto); la varianza estándar-real de R10 sigue siendo **su propia línea**. SPEC §16 se reescribe con la nota de que el Excel no tiene transferencias ni producción (SPEC §3). **CC-011 se recalcula a mano**. 🔴: el consolidado de company da lo mismo antes y después del cambio —las transferencias se cancelan entre sucursales—. La pantalla 25 enseña el desglose, no solo el número. **Se construye como P16-J, inmediatamente antes de la pantalla 25, no antes.** ✅ **Confirmado por el usuario el 2026-09-20:** el `AJUSTE` es **línea propia**, no parte de «no atribuible», así que la varianza de CC-011 es **`4,00 = 2,50 merma − 0,50 ajustes + 2,00 sin explicar`** y la pantalla 25 enseña **tres líneas**. Con ella entra **R15** (CLAUDE.md §6, E24): `varianza (§16) = −mermas_y_ajustes (§18) − diferencia (§18)`, que **no es una tolerancia sino una identidad** —el término de transferencias y producción se cancela al despejar el stock teórico— y se prueba sobre el dataset de CC-010/011/012 **y** sobre el caso conocido de R7. CC-011 ya está reescrito con esos números. **ADENDA del 2026-09-21, sin commit propio —viaja con el de la pantalla 15— y se construye en P16-J:** R15 se enuncia **en cantidades**, `varianza_uso_kg = −mermasYAjustes_kg − diferencia_kg`, porque **en dólares deja de ser identidad en cuanto el precio pagado no es el de referencia**. El dinero se parte: `varianza_uso = varianza_uso_kg × costo_de_uso`, `varianza_precio = Σ(total_cost real − cantidad × costo_de_uso)` con el signo de cada movimiento, y `varianza_total = consumo_real − consumo_teórico = uso + precio`. La pantalla 25 pasa a **cuatro líneas** (merma · ajustes · sin explicar · **precio de compra frente al de referencia**). Caso nuevo **CC-013** —el mes de CC-011 con el arroz a 1,20 y costo de uso 1,00: **24,00 = 4,00 uso + 20,00 precio**— con 🔴 de la identidad en kg y de la suma en dólares; **CC-011 se conserva** con la nota de que su varianza de precio es cero **por construcción**, que es lo que fija que el caso base no enseñe ruido
...
| D-16.198 | (usuario) **El simulacro de restauración por tenant se repite sobre un tenant CON libro y CON un período CERRADO**: sembrar ensayo-b con movimientos y cerrar un mes antes del respaldo. Si `inventory_movement_respeta_periodo_cerrado` rechaza la reinserción, el script resuelve el orden **sin saltarse RLS ni el trigger** (períodos reinsertados abiertos y cerrados al final, en la misma transacción). **El runbook no dice «probado» hasta que este simulacro pase**; evidencia en `CONSTRUCCION.md`. **Hecho en P16-G2**: pasa, y por el camino encontró cuatro cosas —el guardián que faltaba en la catástrofe simulada, el `search_path` vacío de `pg_dump`, la falta de privilegio `TEMP` de `costeo_app` y **INC-030**—. La pared del conteo confirmado la decidió el usuario: **clave foránea diferible** y las líneas antes que su conteo, sin apagar el guardián ni aflojar ningún CHECK |
...
| D-16.200 | (usuario, SPEC §24) **Toda importación que escriba en el libro es reversible como importación**: cada movimiento importado lleva `import_job_id`; `POST /importaciones/:id/anulacion` emite los movimientos de signo contrario en una sola transacción (R3), respeta período cerrado (409 con motivo) y deja la importación en `ANULADA`. 🔴: importar, anular, y el saldo del tenant queda como antes. **Hecho en P16-H**: el endpoint, el hilo `import_job_id`, el estado `ANULADA` con autor, y el 409 del mes cerrado — que sale de la MISMA guarda (`exigirLibroEscribible`) que usa la corrección de un movimiento suelto. De paso destapó **INC-029**: el saldo volvía y el dinero no, porque `total_cost` no lleva signo y nadie restaba las correcciones |
| D-16.203 | (15) **Cuál versión manda hoy lo responde la API, no la pantalla.** El historial llega ordenado por vigencia y `GET /recetas` dice cuál es la vigente; la pantalla solo compara identificadores y **nunca evalúa una fecha contra «hoy»**. Es lo que evita que el frontend reimplemente SPEC §9 —incluido el detalle de que una versión `VOID` gana igual que otra y deja al producto **sin** receta, que es justo el caso que una comparación de fechas se come—. Consecuencia visible: `vigenteId` nulo con versiones en la lista se enseña como «hoy no hay receta vigente», no como lista vacía |
| D-16.204 | (16) **Una sucursal con receta propia NO viene marcada; una sin receta, sí.** Propagar sobre la primera borra el ajuste que alguien hizo ahí, y una casilla marcada de entrada convierte esa pérdida en un descuido de un clic; sobre la segunda no se pierde nada. Y si aun así se marca, **la confirmación la nombra**: «Vas a reemplazar la receta propia de Bodega Norte». Es la previsualización que R11 exige, no un «¿estás seguro?» |
| D-16.205 | (16) **La pantalla de propagación pide `recipe.propagate`**, que hoy solo tiene DUEÑA (P4): un gerente de local no decide la receta de los demás locales, que es lo que R11 llama «permiso de nivel company». La API lo vuelve a exigir en los tres endpoints; esconder la pantalla solo evita enseñar una que va a dar 403 |
| D-16.206 | (17) **El estado vacío del libro va DEBAJO de los filtros, no en su lugar** (`vacio: 'nunca'` y el `Vacio` dentro): el vacío de esta pantalla casi siempre lo causa un filtro, y reemplazar el cuerpo entero se lleva por delante el control que hay que tocar para salir. Es la única pantalla del proyecto que no usa el vacío de `Vista`, y por eso está dicho |
| D-16.207 | (17) **Los filtros viajan en la consulta a la API, nunca se aplican sobre lo cargado.** El libro se pagina por cursor: un `filter` en el navegador daría «la primera página filtrada» con cara de ser el libro entero. Verificado contra la API —`tipo=COMPRA` devuelve 5 filas y la pantalla enseña 5— y con la clave de la tabla atada a la consulta, para que las páginas que «Ver más» añadió con el filtro anterior no se queden debajo |
| D-16.208 | (18) **El permiso vive en la página, no en un `layout.tsx` de sección.** `/movimientos` pide `inventory.read` y `/movimientos/nuevo` pide `inventory.write`, y **BODEGA tiene el segundo y no el primero** (§4.3). Una guardia de sección le habría cerrado la única pantalla de esa sección que de verdad usa. Consecuencias que se escriben para no olvidarlas: la barra lleva **dos entradas** para `/movimientos`, y la de «Registrar» va **primero** porque `entradaDe` devuelve la primera que casa por prefijo; y la salida del formulario es «Volver al inicio» para quien no puede leer el libro |
| D-16.209 | (18) **El formulario pregunta lo que el tipo significa, y no pone ningún signo.** «Cuánto entró», «Cuánto se perdió», «Diferencia (+ sobra · − falta)»: `COMPRA` y `MERMA` mandan la cantidad **en positivo** y el dominio les da su dirección (`movimiento.ts`: «el signo no lo elige quien llama»); `AJUSTE` es el único con signo, y el único campo que admite el menos (`CampoDeCantidadConSigno`). Restar aquí sería la misma regla en dos sitios, que es como nació INC-029 |
| D-16.210 | (18) **La presentación viene preseleccionada: la primera del insumo.** No es comodidad: sin presentación, la tarifa de IVA tiene que salir del grupo, y un grupo que no la define hace que la compra **se rechace** —a propósito, nunca se asume un 15 % (D-16.9)—. Con «Sin presentación» por defecto, quien compra teclearía la factura entera para recibir un error sobre tarifas. Se descubrió intentando registrar una compra de verdad desde la pantalla |
| D-16.211 | (19) **La pantalla de corregir no adelanta el número.** Podría pintar «quedará −25 kg» invirtiendo la cantidad, y sería calcular en el navegador lo que calcula el dominio (`anulacionDe`). Se describe la operación con palabras —«una fila nueva, con la misma fecha y la cantidad al revés»— y el resultado lo enseña el libro. Es la misma línea que D-16.209 y la que INC-029 dejó marcada |
| D-16.212 | (19) **Sin `inventory.write` se ve el movimiento, no un «no tienes acceso».** La página pide `inventory.read` —hay que ver qué se corrige— y el formulario se esconde sin `write`: quien solo lee tiene algo que mirar, no una puerta cerrada. Los dos impedimentos de R3 —ya corregido, o es una corrección— se dicen **antes** de hacer escribir un motivo que se iba a perder; la frontera de verdad sigue siendo el 409 de la API |
| D-16.213 | (21) **La receta da la LISTA de insumos; las cantidades se escriben a mano.** Precargar «lo que la receta manda por este lote» sería multiplicar decimales en el navegador (CLAUDE.md §8) y, peor, invitar a confirmar sin pesar: la varianza de R10 saldría **cero por construcción**, que es justo la mentira que el sistema existe para no contar. La producción registra lo que DE VERDAD entró. Verificado: los tres insumos llegan precargados y los tres **sin cantidad** |
| D-16.214 | (20) **El origen de una transferencia es la sucursal elegida**, y el destino sale de `GET /ubicaciones`, que la API ya filtra por alcance. De ahí sale el hallazgo de abajo: a `BODEGA` la lista le deja una sola sucursal, así que la pantalla dice «no hay otra a la que transferir» — y dice la verdad |
| D-16.215 | (usuario, 2026-09-22) **La transferencia es un sistema replicable de DOS PASOS sobre RUTAS DE ABASTECIMIENTO**, y es la respuesta al hallazgo de la pantalla 20. **No** se le quita `inventory.transfer` a `BODEGA` y **no** se abre el destino a cualquier ubicacion —eso deja que quien despacha escriba en el saldo de otro, y una diferencia en el camino acaba como merma del local que recibe—. La company declara **rutas** (bodega→local, cocina central→local, local↔local); sin ruta no hay transferencia. El **despacho** pide alcance sobre el ORIGEN, deja el producto **en transito** y no toca el destino; la **recepcion** pide alcance sobre el DESTINO y registra lo que llego, con la diferencia como **merma de traslado**, separada de la merma del local. El consolidado ensena el transito como linea propia y **R2 tiene que seguir cuadrando**. Mas: el cierre de periodo define que pasa con los despachos sin recibir (razonado en el ADR, no decidido en silencio); aviso a los N dias; §4.3 sobre la respuesta cruda del despacho; y el despacho guarda los datos de una **guia de remision del SRI** sin emitirla. Los movimientos de un paso del tenant de ensayo son sinteticos y **no se tocan** (R3). **Paquete propio P16-T, con protocolo completo de `apps/api`, despues de P16-E y antes de retomar la pantalla 22. ADR primero, para leerlo antes del codigo: ADR-029, propuesto** |
...
### MODO CIERRE — vigente desde el 2026-09-21 hasta la parada «entrega al piloto»

Decisión del usuario. **Es un régimen temporal y tiene fecha de caducidad: la parada.** Nada de lo
de aquí sobrevive a la entrega al piloto.

| Regla | Qué dice | Qué NO relaja |
|---|---|---|
| **1. Congelación** | Solo interrumpe la pasada un fallo que **corrompa números o datos**, o uno de **seguridad**. Todo lo demás baja a «Después del piloto» y se sigue | El fallo se **escribe** antes de seguir. Congelar no es no verlo |
| **2. Antes del commit** | `audit:types`, `audit:lint`, `audit:forbidden`, `audit:complexity`, `audit:duplication`. **Las pruebas las corre el pre-commit**, no se duplica la auditoría | El pre-commit **no se salta jamás**: `--no-verify` sigue prohibido. Las pruebas corren igual; lo que se quita es correrlas dos veces |
| **3. Commits agrupados** | **20+21** (transferencia y producción) y **26+27** (punto de equilibrio e inventario valorizado). El resto, uno por pantalla | Desviación consciente de «un commit por paquete» de CLAUDE.md §0, **solo en el frontend**, donde no vive ninguna regla de negocio. `apps/api` conserva las 7 fases |
| **4. Documentación** | Por pantalla: **fila en ESTADO + línea en CHANGELOG**. `CONSTRUCCION.md` y `AUDITORIA-RESULTADO.md` **una vez por bloque** (15–24 y 25–27) | **INC y ADR se siguen escribiendo cuando corresponda.** Una incidencia no registrada vuelve |
| **5. Verificación por pantalla** | Sin cambios: **roles · 360 px · tenant cruzado · importes contra `curl`** | Esto es lo que no se toca. Es la única prueba que tiene el frontend |

**Orden de ejecución acordado:**

```
15 → 24  ·  P16-J (con la duda #13 dentro)  ·  25  ·  26+27  ·  P16-E
        ·  [28 y 31, si el piloto tiene más de una sucursal]
        ·  preparación de la parada
```

#### Después del piloto — lo que la congelación aparta

> Aquí baja todo lo que la regla 1 no deja interrumpir. **Una lista vacía al llegar a la parada
> significaría que la congelación no encontró nada, no que nadie miró.**

| # | Qué | De dónde salió |
|---|---|---|
| 3 | **`BODEGA` tiene `inventory.transfer` y no puede transferir nunca.** Una transferencia exige que las **dos** ubicaciones estén en el alcance de la sesión, y `BODEGA` alcanza una sola: la API responde **403 `PERMISO_DENEGADO` «Esa ubicación no está en tu alcance»** aunque la petición se mande saltándose la pantalla (comprobado). La pantalla 20 dice la verdad —«no hay otra sucursal a la que transferir»—, pero el permiso es letra muerta para ese rol, y mover producto de la bodega al local **es justo su trabajo**. Las salidas: (a) el alcance de `BODEGA` incluye los locales a los que surte; (b) `inventory.transfer` sale de `BODEGA` y transfiere quien puede ver las dos puntas. **Afecta al piloto si su bodeguero tiene que mandar producto al local**. ✅ **RESUELTO por el usuario el 2026-09-22 con D-16.215**: ni (a) ni (b), sino el modelo de dos pasos sobre rutas. Se construye en **P16-T**; el diseno esta en **ADR-029**, propuesto | Pantalla 20 |
| 2 | **Un mensaje de la API escrito para un programador, y lo ve el bodeguero.** Al registrar una compra sin tarifa de IVA, el 400 dice: «Mándala en el cuerpo («ivaTarifa»), o fíjala en el artículo de compra (**`PUT /catalogo/articulos/:id`**) o en el grupo del ítem (**`PUT /catalogo/grupos/:id`**)». Contradice la regla de `textos/es.ts` —«nada de *endpoint*, *token* ni *registro*; se dice qué pasó y qué hacer»— y quien lo lee es quien descarga el camión a las seis de la mañana. **La pantalla 18 ya no lo provoca en el caso normal** (preselecciona la presentación del insumo, D-16.210), así que deja de ser urgente, pero el texto sigue mal. **Es un cambio de cadena en la API; conviene barrer los demás mensajes de dominio a la vez** | Pantalla 18 |
| 1 | **El libro enseña el importe SIN signo, y la fila de una corrección se lee al revés.** `total_cost` es una magnitud y el signo vive en `quantity` (ADR-009 §2), así que una corrección de `−100 kg` aparece con `115,00` **en positivo**: quien sume la columna a ojo cuenta `550` de compras en septiembre donde la verdad son `435`. **Es la cara visible de INC-029**, la misma raíz que P16-H y P16-I cerraron en el modelo. **No se arregla en el frontend** —`signo(cantidad) × importe` es una regla del dominio, y calcularla también en el navegador es cómo nació INC-029—: la API tiene que devolver el importe con signo, y `inventory/domain/agregados.ts` ya sabe hacerlo (`importeConSigno`, P16-I). **Mientras tanto la pantalla lo dice con todas las letras** debajo de la tabla, y no enseña ningún total. **Paquete de API corto; cabe en P16-E** | Pantalla 17 |
...
| 9 | **El consolidado de diez ubicaciones ya no cabe en su presupuesto de §5: 940,9 ms contra 800.** Medido con `npm run bench` al cerrar P16-A1, dos corridas con la máquina en reposo (940,9 y 962,6 ms). **No es regresión del paquete**: las consultas y sus recuentos de llamada son los mismos que en P15, y normalizado al «suelo del entorno» que el propio bench mide (validar sesión: 4,5 ms en P15, 6,5 ms hoy) el consolidado cuesta 143,7 suelos frente a los 149,6 de entonces — relativamente, igual. Lo que cambió es la máquina. **Pero el número absoluto es rojo y el umbral no se sube** (AUDITORIA.md I8). **P16-B: 919,4 ms** (suelo 10,0 ms), la primera medición que se sabe hecha sobre el código de su commit —hasta entonces el bench no compilaba el `dist` que medía (INC-007, caso 12)— | P16-A1 | ✅ **DECIDIDA por el usuario el 2026-09-21: opción (c).** Se acepta hasta el piloto, que tiene dos ubicaciones y no toca el camino de diez. **La deuda #2 sigue en pie con su condición de pago intacta.** Lo que esto NO significa: que el umbral se baje. Sigue en 800 ms y la fila I8 de `AUDITORIA.md` sigue en rojo; lo que se difiere es el arreglo, no la medición |
| 11 | **La hoja de conteo (`PUT /conteos/:id/lineas`) es un reemplazo total sin versión** (D-16.129). Si dos personas cuentan la misma hoja a la vez —la cámara fría y el almacén seco—, la segunda en guardar borra lo de la primera. D-16.11 pide concurrencia optimista en los reemplazos totales y D-16.13 no le dio columna; y una versión le daría a quien cuenta un 409 que le haría perder lo contado | P16-C | ✅ **DECIDIDA por el usuario el 2026-09-21: opción (c) para el piloto**, con una regla operativa —**«una hoja, una persona»**— escrita en la **ayuda de la pantalla 23**, no solo en este archivo: un riesgo que solo vive en ESTADO.md no lo lee quien cuenta. **Señal de paso a (a):** que el piloto cuente en paralelo. Entonces se construye el guardado por línea |
...
| 13 | **Un plato vendido sin receta entra a la ingeniería de menú con un margen igual a su venta neta** —sin costo, todo es margen—, así que la matriz lo coloca como estrella o rompecabezas; y suma cero al costo de ventas del food cost real y del consolidado. P16-D solo lo marca en el costeo | P16-D | ✅ **DECIDIDA por el usuario el 2026-09-21: opción (a).** La matriz coloca el plato en **`SIN_DATOS`** —igual que un activo sin PVP— y el resumen cuenta cuántos platos vendidos no tienen receta, como ya cuenta los ítems sin costo. **Se construye DENTRO de P16-J**, con D-16.202, porque toca las mismas vistas. Lo que arregla: hoy un plato sin receta entra como estrella con margen igual a su venta neta, que es la peor mentira posible —le dice al dueño que promocione justo lo que no sabe costear |
...
| 16 | **El `CONSUMO_REAL` de SPEC §16 cuenta como consumo lo que salió por transferencia o por producción.** La fórmula es `inicial + compras − final físico`, y en el Excel eso basta porque **no hay transferencias ni producción**: un solo local, sin dimensión temporal. Aquí sí los hay, así que el local que manda stock a otro ve su food cost real inflado por lo que mandó, y el que lo recibe, deflactado. A nivel de company se compensan. Lo destapó **CC-011** (D-16.201): de 32,00 de varianza, **28,00 son transferencia y producción**, no merma | P16-I · CC-011 | ✅ **DECIDIDA por el usuario el 2026-09-20: opción (a) EXTENDIDA** — `consumo_real` resta las transferencias enviadas y las salidas a producción, suma las recibidas y las entradas al estándar, y la varianza se desglosa en merma registrada y no atribuible. Es **D-16.202**, y se construye como **P16-J, justo antes de la pantalla 25** |
| 10 | **`npm run bench` no lo ejecuta CI.** Corre `npm run audit` y `migrate:verify`, no el bench. El presupuesto del consolidado no lo vigila nada automático — es el riesgo que la propia fila I8 de `AUDITORIA.md` declaraba en voz alta, y que se cumplió: el bench llevaba **dos paquetes sin poder arrancar** (INC-017, recurrencia 1) | P16-A1 | ✅ **DECIDIDA por el usuario el 2026-09-21: NO entra en CI**, por no haber minutos de Actions que gastar. Queda como **bench manual obligatorio en todo paquete que toque lecturas**. El riesgo que esto deja escrito: es una disciplina, no un guardián, y el propio INC-017 demuestra que una disciplina sin check se salta dos paquetes seguidos sin que nadie se entere |
