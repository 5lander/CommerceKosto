...
| **Restricciones de la base sin guarda de dominio** | Un `CHECK` o un trigger hace su trabajo y su error sale como 500 porque nadie lo tradujo. **En P6 no reapareció, y conviene saber por qué:** los triggers append-only del libro resultaron inalcanzables desde la API —no existe ruta que edite un movimiento— y las 18 restricciones nuevas se clasificaron una a una **antes** de escribir el primer endpoint. Reaparecerá en P7 (conteo físico) y P10 (importación), donde sí hay campos que un usuario teclea | **INC-012** · `docs/sistema/guardas-de-dominio.md` |
