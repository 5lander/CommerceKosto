...
| [INC-034](INC-034-una-prueba-que-falla-una-de-cada-pocas-veces.md) | **`read ECONNRESET` dentro de una prueba que pasó hace cinco minutos**, sin aserción fallida y sin relación con lo que la prueba mide. El mismo commit da dos veredictos, y al relanzar pasa | pruebas · build | P16-U6 | 0 |
| [INC-033](INC-033-la-base-arranca-sin-roles-y-ci-nunca-estuvo-en-verde.md) | **`container costeo-db is unhealthy` en CI con la base sana en local**, y en los logs `psql: error: /usr/local/bin/sql/roles.sql: No such file or directory`. El cluster arranca **sin roles**. Solo ocurre con el volumen **vacío**, que en local nunca lo está. **Y el hallazgo mayor: CI nunca había estado en verde** | despliegue · build · base de datos | P16-U | 0 |
| [INC-032](INC-032-un-insumo-sin-precio-vale-cero-y-la-varianza-miente.md) | **Sin error**: un lote se produce y sus insumos quedan valorados en `0,00` porque no tenian precio de referencia a esa fecha. El saldo en kilos cuadra; la varianza de R10 dice que producir salio gratis | dominio · costeo | Pantalla 21 | 0 |
...
| **El arranque de la base solo se prueba EN LIMPIO** | `initdb` corre una única vez, con el volumen vacío. En la máquina de desarrollo el volumen lleva meses creado, así que **todo lo que toque el arranque queda sin probar en local y falla siempre en CI**. Reaparecerá con cada `.sql` nuevo de `initdb/` y con cada rol nuevo. La prueba es levantar la base en un proyecto de compose aparte con volumen propio, no confiar en la auditoría | **INC-033** |
| **Peticiones HTTP lanzadas en paralelo en una suite** | Supertest abre el puerto **perezosamente**: varios `Test` creados en el mismo tick lo intentan a la vez y sale un `ECONNRESET` en una petición al azar. Solo bajo carga, así que el mismo commit da dos veredictos. Reaparecerá en cualquier suite nueva que use `Promise.all` sobre peticiones; ahora lo caza `lote-en-paralelo-sin-servidor-escuchando` | **INC-034** |
| **Lo que git no versiona del archivo** | El contenido está versionado; el **permiso de ejecución** no, porque `core.fileMode=false` en Windows. Un `.sh` que pierde el bit no da error: cambia quién lo interpreta y con ello el valor de `$0`. Misma familia que el CRLF de INC-001, otro eje | **INC-033** · **INC-001** |
